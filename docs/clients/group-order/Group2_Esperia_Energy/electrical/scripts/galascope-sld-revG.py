"""
Galascope BESS — SLD Generator Rev G
Lighthief Cyprus Ltd  |  May 2026

Rev G = Rev F SwS topology + documented dual field coupling options (G1a / G1b / G1c).
See galascope-revG-field-coupling.md and galascope-revF-parallel-feeder-analysis.md.

Rev F/G shared SwS topology (parallel PV feeders — freed SwS bay):
  ABB UniSec SwS (as-built):
    JZ1 — EAC incomer (unchanged)
    JZ2 — combined PV feeder (ex JZ2+JZ3 cables tee to 2×1250 kVA MCTS) [G1]
    JZ3 — repurposed BESS feeder → MV cable → T2/T4 skid RMU → BESS
  G2: single 1250 kVA per MCTS EL00.01.02 — trafo merge N/A; JZ3 BESS only if bay confirmed

Paired pandapower: galascope-pandapower-model-revF.py
Analysis: galascope-revF-parallel-feeder-analysis.md

Drawing numbers:
  G1 (5MW):   LC-G1-SLD-001-G  (Cypriot) / LC-G1-SLD-001-G-IEC  (IEC)
  G2 (2.5MW): LC-G2-SLD-001-G  (Cypriot) / LC-G2-SLD-001-G-IEC  (IEC)

Rev F changes from Rev E:
  - As-built SwS: ABB UniSec 24 kV 16 kA/1s (not Schneider SM6)
  - G1: parallel 2×1250 kVA PV on JZ2; BESS on freed JZ3 (no new cubicle)
  - G2: notes 1×1250 kVA MCTS; same BESS-on-JZ3 concept if bay available

Field coupling (G1, two adjacent 1250 kVA pads ~10 m) — see Rev G for detail:
  G1a — 22 kV outdoor tee (RSTI) + ~10 m MV cable pad-to-pad (no 8DJH swap)
  G1b — Add Siemens 8DJH T-module on nearest MCTS (R+T+T)
  G1c — 8DJH ring cable between MCTS (~10 m) — retire far SwS feeder
"""
...
def draw_field_coupling_notes(msp, cfg, compact=False):
    """Rev G — G1 (~10 m) or G2 (3-pad, ~75–240 m) field coupling options."""
    if not cfg.get("pv_parallel_trafos") and not cfg.get("g2_field_coupling"):
        return
    g2 = cfg.get("g2_field_coupling")
    y0 = 62 if compact else 48
    box_h = 40 if compact else (88 if g2 else 78)
    R(msp, 12, y0 - 2, 396, box_h, layer="DIM", lt="DASHED2")
    title = "REV G — G2 FIELD MV (3 pads, ~75-240m):" if g2 else "REV G — G1 FIELD MV COUPLING:"
    T(msp, title, 14, y0 + box_h - 8, h="sm" if not compact else "xs", layer="ANN")
    if g2:
        lines = [
            "G2a TEE (preferred): MV hub on JZ2 + ~85-95m spurs to T1 & T2 (closest pair ~75m)",
            "G2b 8DJH: hub MCTS R+T1+T2(+T3) — spurs ~95m / up to ~210m if 3 trafos confirmed",
            "G2c RING: interconnect T1-R-T2-R-T3; longest leg ~220-240m — MCTS protection study",
            "Walk-down: EL00.01.02 shows 1x1250/MCTS; satellite 3 pads — likely 2 trafos + 1 other",
            "Retire 1 SwS PV feeder | JZ3 -> BESS | UniSec full (same issue as G1)",
            "galascope-revG-field-coupling-g2.md",
        ]
    else:
        lines = [
            "G1a TEE (preferred): RSTI-58 tee on JZ2; ~10-12m pad-to-pad; NO 8DJH swap",
            "G1b 8DJH EXTEND: add 8DJH T-module -> R+T1+T2; ~10-12m to far trafo HV",
            "G1c 8DJH RING: ~10-12m R1-R2; JZ3 SwS cable removed",
            "JZ3 repurposed -> BESS skid | Trafo: GALA BkAo swap only if mismatch",
        ]
    if compact:
        lines = lines[:4 if g2 else 3]
    step = 7 if compact else 8
    y = y0 + box_h - 18
    for line in lines:
        T(msp, line, 14, y, h="xs", layer="DIM")
        y -= step
...
def draw_cypriot(cfg):
    ...
    T(msp, "ABB UniSec SwS", ex_x, Y_rmu_top+4, h="xs", layer="RMU",
      align=TextEntityAlignment.MIDDLE_CENTER)
    T(msp, "24kV 16kA/1s 200A", ex_x, Y_rmu_top-1, h="xs", layer="DIM",
      align=TextEntityAlignment.MIDDLE_CENTER)
    ...
    T(msp, cfg.get("pv_label", "PV Park"), ex_x, Y_rmu_bot-12, h="xs", layer="ANN",
      align=TextEntityAlignment.MIDDLE_CENTER)
    if cfg.get("pv_parallel_trafos"):
        T(msp, "JZ2 combined PV", ex_x+20, Y_mvbus+2, h="xs", layer="DIM")
        T(msp, "2x1250kVA tee", ex_x, Y_rmu_bot-18, h="xs", layer="DIM",
          align=TextEntityAlignment.MIDDLE_CENTER)
    else:
        T(msp, "JZ2 PV (1x1250)", ex_x+20, Y_mvbus+2, h="xs", layer="DIM")
    ...
    T(msp, "JZ3 BESS feeder", sws_bay_x, Y_rmu_top+2, h="xs", layer="RMU",
      align=TextEntityAlignment.MIDDLE_CENTER)
    T(msp, "(freed bay + 7SJ82)", sws_bay_x, Y_rmu_top-2, h="xs", layer="DIM",
      align=TextEntityAlignment.MIDDLE_CENTER)
    ...
    if cfg.get("pv_parallel_trafos") or cfg.get("g2_field_coupling"):
        draw_field_coupling_notes(msp, cfg, compact=True)
...
def draw_iec(cfg):
    ...
    L(msp, pv_x, Y_arrow, pv_x+4, Y_arrow-8, layer="MV")
    T(msp, f"ABB UniSec SwS {cfg['trafo_hv']:.0f}kV (as-built)",
    ...
    T(msp, cfg.get("pv_label", ""), pv_x, Y_mvbus-40, h="xs", layer="ANN",
      align=TextEntityAlignment.MIDDLE_CENTER)
    if cfg.get("pv_parallel_trafos"):
        T(msp, "JZ2 combined PV", pv_x+18, Y_mvbus+2, h="xs", layer="DIM")
        T(msp, "2x1250kVA MCTS", pv_x, Y_mvbus-40, h="xs", layer="DIM",
          align=TextEntityAlignment.MIDDLE_CENTER)
    else:
        T(msp, "JZ2 1x1250 MCTS", pv_x+18, Y_mvbus+2, h="xs", layer="DIM")

    L(msp, pv_x-16, Y_mvbus, skid_cx+55, Y_mvbus, layer="MV", lw=70)
    T(msp, f"{cfg['trafo_hv']:.0f}kV Bus — Rev F (JZ3 BESS, no new cubicle)",
      pv_x-16, Y_mvbus+3, h="xs", layer="ANN")

    R(msp, sws_x-16, Y_jz_bot, 32, Y_jz_top-Y_jz_bot, layer="SYM", lw=25, lt="DASHED2")
    T(msp, "JZ3 BESS feeder", sws_x, (Y_jz_top+Y_jz_bot)/2+3, h="xs", layer="ANN",
      align=TextEntityAlignment.MIDDLE_CENTER)
    T(msp, "freed bay + 7SJ82", sws_x, (Y_jz_top+Y_jz_bot)/2-2, h="xs", layer="DIM",
      align=TextEntityAlignment.MIDDLE_CENTER)
    ...
    if cfg.get("pv_parallel_trafos") or cfg.get("g2_field_coupling"):
        draw_field_coupling_notes(msp, cfg, compact=False)
...
# pandapower Rev F results (galascope-pandapower-model-revF.py)
CONFIGS = [
    {
        "tag": "G1", "rev": "G", "n_bess": 4, "bess_mwh": 5, "pcs_mw": 1.25,
        ...
        "pv_parallel_trafos": True,
        "pv_label": "PV 5MWp  2x1250kVA MCTS (JZ2 tee)",
        "client": "Esperia Energy Group / Galascope Ltd",
        "project": "Galascope 1  -  5 MW PV + 5 MW / 20 MWh BESS",
        "title": "BESS MV/LV SLD Rev G — JZ2/JZ3 + G1a tee OR G1b 8DJH T+T OR G1c ring",
    },
    {
        "tag": "G2", "rev": "G", "n_bess": 2, "bess_mwh": 5, "pcs_mw": 1.25,
        ...
        "pv_parallel_trafos": False,
        "g2_field_coupling": True,
        "pv_label": "PV 2.5MWp  3 pads T1/T2/T3 (confirm)",
        "client": "Esperia Energy Group / Galascope Ltd",
        "project": "Galascope 2  -  2.5 MW PV + 2.5 MW / 10 MWh BESS",
        "title": "BESS MV/LV SLD Rev G — G2a/b/c field + JZ3 BESS",
    },
]
...
RESULTS_JSON = str(ANALYSIS_DIR / "galascope-pandapower-revF-results.json")
...
print("Generating Rev G SLDs (Cypriot + IEC — dual field coupling options)...")
for cfg in CONFIGS:
    tag = cfg["tag"]
...
print("\nAll Rev G SLDs generated.")
print("Rev G: G1a/G1b/G1c field coupling + JZ3 BESS -> skid RMU")
