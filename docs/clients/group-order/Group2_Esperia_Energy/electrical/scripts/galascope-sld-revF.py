"""
Galascope BESS — SLD Generator Rev F
Lighthief Cyprus Ltd  |  May 2026

Rev F topology (parallel PV feeders — freed SwS bay):
...
Drawing numbers:
  G1 (5MW):   LC-G1-SLD-001-F  (Cypriot) / LC-G1-SLD-001-F-IEC  (IEC)
  G2 (2.5MW): LC-G2-SLD-001-F  (Cypriot) / LC-G2-SLD-001-F-IEC  (IEC)
...
def draw_field_coupling_notes(msp, cfg, compact=False):
    """Rev F/G — G1 (~10 m) or G2 (~75–240 m, 3-pad) field coupling options."""
    if not cfg.get("pv_parallel_trafos") and not cfg.get("g2_field_coupling"):
        return
    g2 = cfg.get("g2_field_coupling")
    y0 = 58 if compact else 52
    box_h = 48 if compact else (58 if g2 else 52)
    h = "xs"
    R(msp, 12, y0 - 2, 396, box_h, layer="DIM", lt="DASHED2")
    if g2:
        T(msp, "G2 FIELD COUPLING — 3 pads T1/T2/T3 (satellite):", 14, y0 + box_h - 8,
          h="sm" if not compact else "xs", layer="ANN")
        lines = [
            "G2a TEE: hub on JZ2 + spurs ~85-95m to T1 & T2 (closest ~75m) — no 8DJH swap",
            "G2b 8DJH: add T-modules at hub MCTS — spurs ~95m / ~210m if 3 trafos",
            "G2c RING: T1-R-T2-R-T3 ~75m + ~220m ring — retire 1 SwS feeder",
            "Confirm: EL00.01.02=1x1250/MCTS; SCADA 2x11 inv — likely 2 trafos + 1 other pad",
            "SwS: JZ2=merged PV | JZ3=BESS | see galascope-revG-field-coupling-g2.md",
        ]
    else:
        T(msp, "G1 FIELD COUPLING (choose one — MCTS approval):", 14, y0 + box_h - 8,
          h="sm" if not compact else "xs", layer="ANN")
        lines = [
            "G1a TEE: RSTI tee on JZ2 + ~10-12m pad-to-pad (no 8DJH swap)",
            "G1b 8DJH: add T-module R+T+T on nearest MCTS + ~10m spur",
            "G1c RING: ~10m R-R between MCTS; JZ3 SwS cable removed",
        ]
        if not compact:
            lines.append("SwS: JZ2=PV | JZ3=BESS | galascope-revG-field-coupling.md")
    if compact:
        lines = lines[:3]
    step = 7 if compact else 8
    y = y0 + box_h - 18
    for line in lines:
        T(msp, line, 14, y, h=h, layer="DIM")
        y -= step
...
CONFIGS = [
    {
        "tag": "G1", "rev": "F", "n_bess": 4, "bess_mwh": 5, "pcs_mw": 1.25,
        ...
        "title": "BESS MV/LV SLD Rev F — JZ2 PV merge + JZ3 BESS (field opt G1a/b/c)",
    },
    {
        "tag": "G2", "rev": "F", "n_bess": 2, "bess_mwh": 5, "pcs_mw": 1.25,
        ...
        "pv_label": "PV 2.5MWp  2-3 pads (confirm trafos)",
        "client": "Esperia Energy Group / Galascope Ltd",
        "project": "Galascope 2  -  2.5 MW PV + 2.5 MW / 10 MWh BESS",
        "title": "BESS MV/LV SLD Rev F — JZ2/JZ3 + G2a/b/c (~75-95m spurs)",
    },
]
...
print("Generating Rev F SLDs (Cypriot + IEC — parallel PV / JZ3 BESS)...")
for cfg in CONFIGS:
    tag = cfg["tag"]
    g_label = "1" if tag == "G1" else "2"

    cfg["drwno"] = f"LC-G{g_label}-SLD-001-F"
    cfg["date"] = "18/05/2026"
    doc_cy = draw_cypriot(cfg)
    path_cy = f"{OUTPUT_DIR}/LC-G{g_label}-SLD-001-F_Cypriot.dxf"
    doc_cy.saveas(path_cy)
    print(f"  Saved: {path_cy}")

    cfg["drwno"] = f"LC-G{g_label}-SLD-001-F-IEC"
    doc_iec = draw_iec(cfg)
    path_iec = f"{OUTPUT_DIR}/LC-G{g_label}-SLD-001-F-IEC.dxf"
...
print("\nAll Rev F SLDs generated.")
print("Rev F: JZ2 combined PV (G1) + JZ3 BESS feeder -> MV cable -> skid RMU")
