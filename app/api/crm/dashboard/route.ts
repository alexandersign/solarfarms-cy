...
export async function GET(request: NextRequest) {
  ...
  const nameToEmail: Record<string, string> = {
    ...
    'Artur Rull':           'a.rull@lighthief.com',
    'Andreas Christoforou': 'office@lighthief.com',
    // legacy entries — keep for historical activity_feed records
    ...
    'Artur':     'a.rull@lighthief.com',
  }

  // Seed the map with all active CRM users so they always appear even with 0 activity
  const CRM_USER_NAMES = ['Alexander Papacosta', 'Costas Hadjikyriacou', 'Artur Rull', 'Andreas Christoforou']
  ...
  const HUMAN_AUTHORS = new Set([...CRM_USER_NAMES, 'Office', 'Alexander', 'Zinovia', 'Costas', 'Andreas', 'Artur'])
